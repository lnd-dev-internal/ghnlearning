...
with open('c:\\Users\\Huawei\\.gemini\\antigravity\\scratch\\ghn-portal\\dien-gia.html', 'r', encoding='utf-8') as f:
    html = f.read()

# Replace CSS
oldCss = """.speaker-card-grid {
      display: grid;
...
if oldCss in html:
    ...
    # Use regex
    html = re.sub(r'\.speaker-card-grid \{.*?\.spk-card-body p \{[^\}]*\}', newCss, html, flags=re.DOTALL)


html_match = re.search(r'<div class="speaker-grid" style="display: grid; grid-template-columns: repeat\(2, 1fr\); gap: 32px; max-width: 900px; margin: 0 auto;">[\s\S]*?</div>\s*</section>', html)

newHtml = """<div class="speaker-grid-new">
        <!-- Row 1 -->
...
if html_match:
    html = html.replace(html_match.group(0), newHtml)
...
with open('c:\\Users\\Huawei\\.gemini\\antigravity\\scratch\\ghn-portal\\dien-gia.html', 'w', encoding='utf-8') as f:
    f.write(html)
